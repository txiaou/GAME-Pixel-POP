...
export const BubbleMachine: React.FC<BubbleMachineProps> = ({ isShooting }) => {
  return (
    <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-48 h-28 flex flex-col items-center">
      {/* Nozzle */}
      <div className={`w-12 h-6 bg-gray-500 border-2 border-gray-400 rounded-t-md transition-transform duration-100 ease-in-out ${isShooting ? 'scale-y-75 -translate-y-1' : ''}`}></div>
      {/* Body */}
